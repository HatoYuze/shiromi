...
import com.github.hatoyuze.shiromi.protocol.api.LuoguApiException
import com.github.hatoyuze.shiromi.protocol.api.LuoguHttpClient
import com.github.hatoyuze.shiromi.protocol.api.LuoguTags
import com.github.hatoyuze.shiromi.protocol.api.ProblemType
...
class LuoguApiTest {
    ...
    @Test fun `fromId correct`() {
        ...
        assertEquals(DifficultyLevel.IMPROVEMENT, DifficultyLevel.fromId(5))
        assertEquals(DifficultyLevel.PROVINCIAL, DifficultyLevel.fromId(7))
    }
    ...
    @Test fun `missing credentials ok for tool reg`() = runTest {
        val host = toolHost { installLuoguTools(LuoguApi()) }
    ...
    @Test fun `installLuoguTools registers 9 tools`() {
        val h = toolHost { installLuoguTools(LuoguApi()) }
...
private fun mockApi(cache: CacheStorage, handler: MockRequestHandler): LuoguApi {
    val mockClient = HttpClient(MockEngine(handler))
    val http = LuoguHttpClient(mockClient)
...
class MemoryCacheStorage : CacheStorage {
    ...
    override suspend fun listEntries(): List<CacheEntry> =
        entries.map { CacheEntry(it.key, it.value.size.toLong(), 0L) }
}
