...
object SampleSerializer : KSerializer<Sample> {
    ...
    override fun deserialize(decoder: Decoder): Sample {
        ...
        return when {
            ...
            element is JsonArray && element.size >= 2 -> {
                Sample(
    ...
    override fun serialize(encoder: Encoder, value: Sample) {
        val array = buildJsonArray {
            add(JsonPrimitive(value.input))
            add(JsonPrimitive(value.output))
            if (value.description != null) add(JsonPrimitive(value.description))
        }
