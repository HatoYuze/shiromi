...
internal class JvmNavigationInterceptor(instance: WebViewBridgePanel) : Interceptor {
    // Shiromi fork: handlers are registered/unregistered on the UI thread but invoked on
    // the native WebView thread (NativeBridge.onNavigationInterceptorCallback), so the
    // container must be thread-safe to avoid ConcurrentModificationException on the JNI path.
    private val handlers: MutableMap<Int, MutableSet<InterceptorHandler>> = ConcurrentHashMap()
    ...
    override fun registerNavigationInterceptor(index: Int, handler: InterceptorHandler): CloseHandle {
        val priorityHandlers = handlers.getOrPut(index) { ConcurrentHashMap.newKeySet() }
    ...
    override suspend fun evaluateScript(script: String): String? {
        ...
        return suspendCancellableCoroutine { c ->
            SwingUtilities.invokeLater {
                ...
                // Shiromi fork: never log raw results — evaluateScript can return session
                // cookies (document.cookie). Log only the length.
                LoggerReceiver.log(
                    LoggerReceiver.Level.VERBOSE,
                    TAG,
                    "evaluateScript: result length=${result?.length ?: -1}",
                )
                c.resume(result)
            }
        }
