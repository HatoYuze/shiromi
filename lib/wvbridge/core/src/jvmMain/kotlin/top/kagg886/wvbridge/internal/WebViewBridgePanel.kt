...
internal class WebViewBridgePanel(
    ...
    public fun evaluateScript(script: String): String? {
        ...
        LoggerReceiver.log(LoggerReceiver.Level.VERBOSE, TAG, "evaluateScript: result length=${result?.length ?: -1}")
        return result
    }

    /**
     * Reads the cookie header string (`name=value; name=value`, HttpOnly included)
     * for [url] from the native engine cookie store.
     *
     * Shiromi fork addition (see VENDOR.md). Cookie values are never logged.
     */
    public fun getCookieString(url: String): String? {
        LoggerReceiver.log(LoggerReceiver.Level.INFO, TAG, "getCookieString: url=$url")
        val result = getCookieString(handle, url)
        LoggerReceiver.log(LoggerReceiver.Level.VERBOSE, TAG, "getCookieString: result length=${result?.length ?: -1}")
        return result
    }
    ...
    private external fun getCookieString(webview: Long, url: String): String?
